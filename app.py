...
def lostpassword():
	if request.method == 'POST':
		...
		if results > 0:
			...
			msg1 = Message('FaceBit - OTP Verification for Lost Password', sender = sender, recipients = [lpemail])
...
def share_details_emails():
	if request.method == 'POST':
		...
		msg1 = Message('EXAM DETAILS - FaceBit', sender = sender, recipients = [emailssharelist])
